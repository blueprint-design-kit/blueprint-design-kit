...
export async function testExpectations(options?: BlueprintSystemOptions['testingOptions'], filter?: string) {
    const { serverCommand = '', serverUrl = '', timeout = 0, browserLauncher } = extendOptions(options);
    ...
    try {
        ...
        results = await testInPlaywright(serverUrl, handleError, { filter, timeout, browserLauncher });
    } catch (err) {
