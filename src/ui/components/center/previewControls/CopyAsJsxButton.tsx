...
function convertHtmlToJsx(html: string) {
    const wrapperNode = 'conversionwrapper';
    // @ts-expect-error - parseHtmlToReact doesn't have correct types for default export
    const reactElement: string | JSX.Element | JSX.Element[] = parseHtmlToReact(`<${wrapperNode}>${html}</${wrapperNode}>`);
    // @ts-expect-error - react-element-to-jsx-string doesn't have correct types for default export
    const convertToJsxString = typeof reactElementToJSXString === 'function' ? reactElementToJSXString : reactElementToJSXString.default;
    let jsxString = convertToJsxString(reactElement);
    if (jsxString.indexOf(`<${wrapperNode}>`) === 0) {
        jsxString = jsxString.replace(new RegExp(`^<${wrapperNode}>`), '').replace(new RegExp(`</${wrapperNode}>$`), '')
    }
    return jsxString;
}
...
export function CopyAsJsxButton({ selectorId }: { selectorId: string }) {
    ...
    async function copyJsxToClipboard() {
        try {
            const html = document.getElementById(selectorId)?.innerHTML;
            if (!html) { throw new Error('Element not found.'); }
            const jsxString = convertHtmlToJsx(html);
            await navigator.clipboard.writeText(jsxString);
            flashFeedback('JSX copied to clipboard');
        } catch (err) {
            flashFeedback('Failed to copy');
    ...
    return <>
        <div
            className="blueprint-layout-preview-control-copy-jsx"
            onClick={copyJsxToClipboard}
        >
            <svg className="blueprint-copy-jsx-icons" width="26" height="26" viewBox="-160 -160 780 780" xmlns="http://www.w3.org/2000/svg" fillRule="evenodd" clipRule="evenodd">
                <title>Copy JSX to clipboard</title>
                <path fill="currentColor" strokeWidth="18" stroke="currentColor"
            ...
            <CSSTransition
                ...
                unmountOnExit
            >
                <div ref={ref} className="blueprint-layout-preview-control-copy-jsx-feedback">{feedback}</div>
            </CSSTransition>
        </div>
    </>;
};
